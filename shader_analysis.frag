...
void main (void) {
  ...
  vec2 center_circle = vec2(0.0, 0.0);
  ...
  for (int i = 0; i < 4; ++i)
	...
	if ((!depth_test) || (pixelB[i].x <= zmax)) {
	  ...
	  if (pixelA[i].w > max_radius)
	    max_radius = pixelA[i].w;

	  center_circle = (pixelB[i].zw + gather_pixel_desloc[i].xy);
  ...
  if (valid_pixels > 0.0) {
    ...
    //compute radius, dist to one valid circle + max radius
    //bufferA.w = length(bufferB.zw - center_circle) + max_radius;
